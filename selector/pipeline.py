...
Used by `selector.run` (CLI) and `backend.dossier_screen` (deploy API).
"""
...
def run_pipeline(
    ...
    *,
    ...
    """Run dossier-based screening for one strategy.

    Returns a payload dict with funnel stats, market_context, and FinalPicks
    result. Optionally writes intentions/<strategy>_<date>.json.
    """
    ...
    log.info("pipeline start strategy=%r dossiers=%d budget=₹%s", strategy, dossier_count, budget)
    ...
    if not survivors:
        ...
        result = select_final(scored, account, market_context, per_stock_cap_pct=cap_pct, reasoning=trail)
